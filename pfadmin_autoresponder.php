...
//    $rcmail->output->send('pfadmin_autoresponder.pfadmin_autoresponder');
  ...
  function pfadmin_autoresponder_save()
  {
    ...
    $enabled     = rcube_utils::get_input_value('_autoresponderenabled', RCUBE_INPUT_POST);
    if(!$enabled)
      $enabled = 0;
    if(isset($_SESSION['dnsblacklisted']) && $_SESSION['dnsblacklisted'] != 'pass')
      $enabled = 0;
    $subject     = rcube_utils::get_input_value('_autorespondersubject', RCUBE_INPUT_POST);
    $body        = rcube_utils::get_input_value('_autoresponderbody', RCUBE_INPUT_POST);
    $date        = rcube_utils::get_input_value('_autoresponderdate', RCUBE_INPUT_POST);
    $datefrom      = rcube_utils::get_input_value('_autoresponderdatefrom', RCUBE_INPUT_POST);
  ...
  function pfadmin_autoresponder_form()
  {
    ...
    if(!empty($date) AND $date !="0" AND $date !="NULL") {
	$date = date('d.m.Y',$date);
    }else{
	$date = "";
    }

    if(!empty($datefrom) AND $datefrom !="0" AND $datefrom !="NULL") {
  ...
  private function _save($user,$enabled,$subject,$body,$date, $datefrom)
  {
    ...
    $sql = str_replace('%d',  (strtotime($date)==0 ? 'NULL' : $db->quote(strtotime($date),'text')), $sql);           
    //$sql = str_replace('%f',  preg_match('/NULL|now/', $datefrom) ? $datefrom : $db->quote($datefrom,'text'), $sql);           
    $sql = str_replace('%f',  (strtotime($datefrom)==0 ? 'NULL' : $db->quote(strtotime($datefrom),'text')), $sql);           
